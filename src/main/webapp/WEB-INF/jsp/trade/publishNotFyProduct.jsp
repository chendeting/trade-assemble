...
<head>
    ...
    <link rel="stylesheet" href="/static/newhope/html/css/hj_fabu_(feifangyang)_4step.css"/>
...
<!--猪场渲染模板-->
<script type="text/html" id="pigFaramList">
 	<ul>
		{{each data as pigFaram index}}
        	<li pigFaramId="{{pigFaram.id}}" onclick="queryPigFaramInfo('{{pigFaram.id}}')">{{pigFaram.name}}</li>
		{{/each}}
    </ul>
</script>
<!--切换竞拍日期统一定价渲染模板-->
...
<form id="publishForm" action="/trade/product/saveTradeProduct" method="post">
	...
	<section>
		...
		<input type="hidden" id="creator" name="${tradeInfo.creator}"/>
		<input type="hidden" id="pigFaramId" name="pigFaramId" value="${tradeInfo.pigFaramId}"/>
	    ...
	    <div id="step1_chooseCircle"  >
	        ...
	        <div style="display: flex;justify-content: center"><p class="hrefP"><a href="/trade/circle/list?type=choice&source=${tradeInfo.source}&productId=${tradeInfo.id}" class="chooseHref">${empty circleEx.name ? '选择圈子' : circleEx.name }<img src="/static/newhope/html/images/ic_list_detail.png"/></a></p></div>
	        <p class="next-btn"><input type="button" style="color:#fff" class="nextStep" value="下一步"></p>
	    </div>
	            ...
	            <div class="time1">
	                ...
	                <div class="weui_row actionDate findinput">
	                    <div class="weui_cell_bd weui_cell_primary">
	                    	<c:if test="${!empty tradeInfo.bidTime }">
	                    	...
	                    	<c:if test="${empty tradeInfo.bidTime }">
	                        	<input class="weui_input" id="IdauctionDate" name="bidDateStr" value="<%=nowDate %>" type="text">
	                    	</c:if>
	        ...
	        <ul>
	            ...
	            <li>
	                <div class="weui-row weui-no-gutter">
	                    ...
	                    <div class="weui-col-75">
	                        <input type="number" class="wholeNum" onkeyup="limitInputLength(this,9);" name="marketingNumber" value="${tradeInfo.marketingNumber }" placeholder="请输入出栏数量">
	                        <span>头</span>
	                    </div>
	                </div>
	            </li>
	            ...
	            <li>
	                <div class="weui-row weui-no-gutter">
	                    ...
	                    <div class="weui-col-75">
	                        <input type="number" class="decimalNum" name="preAvgWeight" value="${tradeInfo.preAvgWeight }" placeholder="请输入预估均重">
	                        <span>KG</span>
	                    </div>
	                </div>
	            </li>
	            <li>
	                <div class="weui-row weui-no-gutter">
	                    ...
	                    <div class="weui-col-75">
	                        ...
	                        <input type="text" id="kongliao" value="${tradeInfo.controllerFeedstuffInfo.displayName }" placeholder="控料情况">
	                        <i class="fa fa-caret-down" aria-hidden="true"></i>
	                    </div>
	                </div>
	            </li>
	        ...
	        <ul>
	            <li>
	                <div class="weui-row weui-no-gutter">
	                    ...
	                    <div class="weui-col-75">
	                    	<c:if test="${!empty tradeInfo.marketingDate }">
	                        	<input type="text" id="jiaogeDate" name="marketingDate" value="<fmt:formatDate value='${tradeInfo.marketingDate }' pattern='yyyy-MM-dd'/>" placeholder="请选择交割日期">
	                    	</c:if>
	                    	<c:if test="${empty tradeInfo.marketingDate }">
	                        	<input type="text" id="jiaogeDate" name="marketingDate" value="<%=nextDate %>" placeholder="请选择交割日期">
	                    	</c:if>
	                        <i class="fa fa-caret-down" aria-hidden="true"></i>
	                    </div>
	                </div>
	            ...
	            <li>
	                <div class="weui-row weui-no-gutter">
	                    ...
	                    <div class="weui-col-75">
	                        <input type="text" id="pigCircleName" onkeyup="sendKeyWord(event)" value="${tradeInfo.pigFarmInfoExModel.name }" placeholder="请选择猪场名称">
	                        <i class="fa fa-caret-down" aria-hidden="true" onclick="$(this).prev().click();"></i>
	                    </div>
	                    <div class="selectPig">
	                    </div>
	                </div>
	            ...
	            <li>
	                ...
	                <div class="weui-col-75">
	                    <input class="addr" type="text" id="mapAddress" readonly="readonly" placeholder="请进行定位" name="mapAddress" value="${tradeInfo.pigFarmInfoExModel.mapAddress }">
...
$(function(){
	...
	var IdauctionDate = $("#IdauctionDate").val().replace(/\-/g, "/");
	
	//转化为苹果支持的格式
// 	currentDate = currentDate.replace(/\\/g,",");

	// 竞拍时间
	//$("#jiaogeDate").val(notSelect);
	...
	$("#jiaogeDate").calendar({
		minDate : IdauctionDate,
	    onChange: function (p, values, displayValues) {
	    	//$("#jiaogeDate").data("calendar").params.minDate = currentDate;
	    },
	    onOpen:function(){
	    	//$("#jiaogeDate").data("calendar").params.minDate = currentDate;
	    }
